...
Java_com_valhalla_valhalla_ValhallaKotlin_traceRoute(JNIEnv *env,
    ...
    try {
        ...
        printf("[ValhallaActor] traceRoute valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();

        result = "{\"code\":" + code + ",\"message\":\"" + message + "\"}";
    } catch (const std::exception &err) {
        printf("[ValhallaActor] traceRoute std::exception: %s\n", err.what());
        result = "{\"code\":-1,\"message\":\"" + std::string(err.what()) + "\"}";
    } catch (...) {
        printf("[ValhallaActor] traceRoute unknown exception");
        result = "{\"code\":-1,\"message\":\"unknown exception\"}";
    }
...
Java_com_valhalla_valhalla_ValhallaKotlin_traceAttributes(JNIEnv *env,
    ...
    try {
        ...
        printf("[ValhallaActor] traceAttributes valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();

        result = "{\"code\":" + code + ",\"message\":\"" + message + "\"}";
    } catch (const std::exception &err) {
        printf("[ValhallaActor] traceAttributes std::exception: %s\n", err.what());
        result = "{\"code\":-1,\"message\":\"" + std::string(err.what()) + "\"}";
    } catch (...) {
        printf("[ValhallaActor] traceAttributes unknown exception");
        result = "{\"code\":-1,\"message\":\"unknown exception\"}";
    }
